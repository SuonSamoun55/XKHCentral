    ...
    <main class="content-scroll">
        ...
        <div class="dashboard" id="dashboardGrid">
            ...
            <section class="card area-recent recent-card">
                <div class="card-head">
                    <h3>Recent order</h3>
                    @include('POSViews.POSUserViews.Daskboard.partials.year_month_filter')
                </div>
                <div class="order-list">
                    @forelse($recentOrders as $order)
                        @php
                            ...
                            $itemName =
                                $firstOrderItem?->item?->display_name ??
                                ($firstOrderItem?->item_name ?? 'Unknown item');
                            ...
                            <div class="order-left">
                                <div class="order-thumb-stack {{ $isStacked ? 'stacked' : 'single' }}">
                                    @forelse($thumbItems as $thumb)
                                        <div class="thumb-slot">
                                            @if ($thumb['image'])
                                                ...
                                                <span class="item-avatar-fallback"
                                                    style="display:none;">{{ $thumb['initial'] }}</span>
                                            @else
                                    ...
                                    @empty
                                        <div class="thumb-slot">
                                            <span
                                                class="item-avatar-fallback">{{ $itemName ? mb_strtoupper(mb_substr($itemName, 0, 1)) : '?' }}</span>
                                        </div>
                                    @endforelse
                                </div>
                                <div>
                    ...
                    @empty
                        <img class="NoOrderImage" src="/images/pos/No recent Orders.png" alt="" loading="lazy">
                    @endforelse
                </div>
                                ...
                                <div class="notif-badge">
                                    @if ($notification->display_icon === 'admin')
                                        @if ($notification->sender_profile_image)
                                            <img src="{{ $notification->sender_profile_image }}"
                                                alt="{{ $notification->title ?? 'Notification' }}" loading="lazy"
                ...
                <div class="order-list">
                    @forelse($topPurchasedItems as $item)
                        @php
                            ...
                            $boughtItemImage =
                                $resolveTopImg($item->custom_image_url ?? null) ??
                        ...
                        <div class="order-item">
                            <div class="order-left">
                                    ...
                                    <div class="thumb-slot">
                                        @if ($boughtItemImage)
                                            <img src="{{ $boughtItemImage }}" alt="{{ $boughtItemName }}"
                                                ...
                                                onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex';">
                                            <span class="item-avatar-fallback"
                                                style="display:none;">{{ $boughtItemInitial }}</span>
                                        @else
                                ...
                                <div>
                                    <div class="order-name">{{ \Illuminate\Support\Str::limit($boughtItemName, 32) }}
                                    </div>
                    ...
                    @empty
                        <img class="no_top_item" src="{{ asset('/images/pos/no top item in the list.png') }}"
                            alt="no top item" loading="lazy">
                    @endforelse
                </div>
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                callbacks: {
                                    ...
                                    label: (item) =>
                                        `Money: $${Number(item.raw || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                                }
                            }
                        },
                        scales: {
