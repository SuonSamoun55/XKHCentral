...
@push('styles')
    <link rel="stylesheet"
        href="{{ asset('css/views/POSViews/POSAdminViews/Orders/show.css') }}?v={{ filemtime(public_path('css/views/POSViews/POSAdminViews/Orders/show.css')) }}">
@endpush
        ...
        <div class="approval-page-inner">
            ...
            <div class="header-title">
                ...
                <a href="{{ route('orders.report.preview', $order->id) }}"
                    style="margin-left:auto; display:inline-flex; align-items:center; gap:6px; padding:8px 16px; border-radius:8px; background:#0EA8B2; color:#fff; font-size:13px; font-weight:600; text-decoration:none;">
            ...
            <div class="info-cards-row">
                ...
                <div class="info-card">
                    <div class="info-card-text">
                        ...
                        <div class="info-card-value">{{ $order->user ? ucwords($order->user->name) : 'N/A' }}</div>
                    </div>
                    <div class="info-card-icon">
                ...
                <div class="info-card">
                    <div class="info-card-text">
                        ...
                        <div class="info-card-value">
                            {{ \Carbon\Carbon::parse($order->checked_out_at ?? $order->created_at)->format('d-m-y') }}</div>
                    </div>
                    <div class="info-card-icon">
                    ...
                    <tbody>
                        @forelse($order->items as $item)
                            @php
                                ...
                                $productImage = $orderLineImage($item);

                                $productName = $item->item_name ?? 'Item Name';
                                $unitLabel = strtoupper($itemModel->base_unit_of_measure_code ?? '');
                                $variantLabel =
                                    optional($item->itemVariant)->description ?? ($item->variant_description ?? null);
                            ...
                            <tr>
                                <td data-label="Product Name">
                                    <div class="desc-cell">
                                        ...
                                        <div class="desc-text-wrap">
                                            ...
                                            @endif
                                            @if ($unitLabel)
                                                <div class="desc-sub">Unit: {{ $unitLabel }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td data-label="Price" class="col-center">${{ number_format($item->unit_price ?? 0, 2) }}
                                </td>
                                ...
                                <td data-label="Subtotal"><strong>${{ number_format($item->line_total ?? 0, 2) }}</strong>
                                </td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="approval-footer">
                @if (($order->status ?? '') === 'pending')
                    <button type="button" class="reject-link-btn" data-bs-toggle="modal" data-bs-target="#rejectModal">
                        Reject Order
                    </button>

                    <form action="{{ route('admin.orders.confirm', $order->id) }}" method="POST"
                        class="complete-approval-form js-approve-order-form">
                        @csrf
            ...
            <div class="mobile-order-detail">
                <div class="mobile-customer-card">
                    <div class="mobile-customer-top">
                        <img class="mobile-customer-avatar"
                        ...
                        <div>
                            <div class="mobile-customer-name">{{ $order->user ? ucwords($order->user->name) : 'N/A' }}</div>
                ...
                <div class="mobile-section-card">
                    <div class="mobile-section-header">
                        ...
                        <span class="mobile-section-count">{{ $order->items->count() }}
                            {{ \Illuminate\Support\Str::plural('item', $order->items->count()) }}</span>
                    </div>
                    ...
                    <div class="mobile-order-items-list">
                        @forelse($order->items as $item)
                            @php
                                ...
                                $mobileProductImage = $orderLineImage($item);
                                $mobileProductName = $item->item_name ?? 'Item Name';
                                $mobileUnitLabel = strtoupper($mobileItemModel->base_unit_of_measure_code ?? '');
                                $mobileVariantLabel =
                                    optional($item->itemVariant)->description ?? ($item->variant_description ?? null);
                            @endphp
                            <div class="mobile-order-item-row">
                                <img class="mobile-order-item-thumb" src="{{ $mobileProductImage }}"
                                    alt="{{ $mobileProductName }}"
                                    onerror="this.onerror=null;this.src='{{ asset('images/no-image.png') }}';">
                                <div class="mobile-order-item-info">
                                    ...
                                    @endif
                                    @if ($mobileUnitLabel)
                                        <div class="mobile-order-item-variant">Unit: {{ $mobileUnitLabel }}</div>
                                    @endif
                                </div>
                                <div class="mobile-order-item-price-wrap">
                                    <div class="mobile-order-item-price">${{ number_format($item->unit_price ?? 0, 2) }}
                                    </div>
                ...
                <div class="mobile-section-card">
                    ...
                    <div class="mobile-cost-row">
                        ...
                        <span
                            class="mobile-cost-positive">{{ $shipping > 0 ? '$' . number_format($shipping, 2) : 'Free' }}</span>
                    </div>
                    <div class="mobile-cost-row">
                        <span>VAT</span>
                        <span
                            class="mobile-cost-positive">{{ $taxes > 0 ? '$' . number_format($taxes, 2) : 'Free' }}</span>
                    </div>
                ...
                @if (($order->status ?? '') === 'pending')
                    ...
                    <a href="{{ route('orders.report.preview', $order->id) }}" class="mobile-view-report-btn mobile-view-report-btn-block">
                        <i class="bi bi-file-earmark-pdf"></i> View Report
                    </a>

                    <div class="mobile-approval-actions">
                        <form action="{{ route('admin.orders.confirm', $order->id) }}" method="POST"
                            class="mobile-approve-order-form js-approve-order-form">
                            @csrf
                        ...
                        <button type="button" class="mobile-reject-order-btn" data-bs-toggle="modal"
                ...
                @else
                    <div class="mobile-status-actions">
                        ...
                        <a href="{{ route('orders.report.preview', $order->id) }}" class="mobile-view-report-btn">
                            <i class="bi bi-file-earmark-pdf"></i> View Report
                        </a>
                    </div>
                @endif
                ...
                <div class="modal-content remark-modal-content">
                    <form action="{{ route('admin.orders.cancel', $order->id) }}" method="POST">
                        ...
                        <div class="modal-body remark-modal-body">
                            <textarea name="note" class="remark-textarea" required>{{ old('note') }}</textarea>
