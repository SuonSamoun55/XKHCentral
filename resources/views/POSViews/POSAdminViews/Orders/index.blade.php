...
@push('styles')
<link rel="stylesheet" href="{{ asset('css/views/POSViews/POSAdminViews/Orders/index.css') }}?v={{ filemtime(public_path('css/views/POSViews/POSAdminViews/Orders/index.css')) }}">
@endpush
@section('content')
<div class="approval-page">
    ...
    <div class="mobile-top-tools">
        ...
        <form method="GET" action="{{ route('admin.orders.index') }}" class="mobile-date-form" id="mobileDatePanel">
            ...
            @endif
            <input type="date" name="date" id="mobileDateInput" value="{{ request('date') }}" onchange="this.form.submit()">
        </form>
    ...
    {{-- Mobile-only: mirrors the New Orders tab count as a banner, matching the desktop tab badge --}}
    @if(($tab ?? 'new') === 'new')
                    ...
                    <table class="approval-table">
                        ...
                        <tbody>
                            @foreach($orders as $order)
                                @php
                                    // The action matching the order's *current* status — covers both the
                                    // confirm and the reject path, so "Approved By"/Remark are correct
                                    // either way (a rejected order has no 'confirmed' action to find).
                                    $orderAction = $order->actions->first(fn ($a) => strtolower($a->status ?? $a->action_type) === strtolower($order->status));
                                @endphp
                                <tr class="clickable-row" data-href="{{ route('admin.orders.show', $order->id) }}">
                                    ...
                                    @if (($tab ?? 'new') === 'approved')
                                        ...
                                        <td>
                                            @if ($orderAction)
                                                <div class="approved-by-name">{{ $orderAction->actionBy->name ?? 'Admin' }}</div>
                                            @else
                                        ...
                                        <td>
                                            <span class="status-pill-outline">
                                                <i class="bi {{ in_array($order->status, ['cancelled','rejected']) ? 'bi-x-circle' : 'bi-check-circle' }}"></i>
                                    ...
                                    @else
                                        ...
                                        <td>
                                            <div class="price-text">${{ number_format($order->total_amount ?? 0, 2) }}</div>
                                        </td>

                                        <td>
                                            <span class="status-pill status-{{ strtolower($order->status) }}">
                                                @if ($order->status !== 'pending')
                                                    <i class="bi {{ in_array($order->status, ['cancelled','rejected']) ? 'bi-x-circle-fill' : 'bi-check-circle-fill' }}"></i>
                                                @endif
                                        ...
                                        <td onclick="event.stopPropagation();">
                                            <div class="action-group">
                                                <form action="{{ route('admin.orders.confirm', $order->id) }}" method="POST" class="action-inline-form js-approve-order-form">
                                                    @csrf
                ...
                @foreach($orders as $order)
                    @php
                        ...
                        $statusIcon = in_array($order->status, ['cancelled', 'rejected'])
                            ? 'bi-x-circle-fill'
                            : (in_array($order->status, ['confirmed', 'completed', 'approved']) ? 'bi-check-circle-fill' : null);
                    ...
                    <div class="mobile-order-card">
                        <div class="mobile-order-top">
                            <img
                                class="mobile-order-avatar"
                                src="{{ $order->user->profile_image_display ?? 'https://ui-avatars.com/api/?name=' . urlencode($order->user->name ?? 'User') . '&background=17bfd0&color=fff' }}"
                                alt="{{ $order->user->name ?? 'User' }}"
                                onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name={{ urlencode($order->user->name ?? 'User') }}&background=17bfd0&color=fff';"
                            >
                            <div class="mobile-order-name-wrap">
                                <div class="mobile-order-name">{{ $order->user->name ?? 'N/A' }}</div>
                                <span class="status-pill status-{{ strtolower($order->status) }}">
                                    @if($statusIcon)<i class="bi {{ $statusIcon }}"></i>@endif
                                    {{ $statusLabel }}
                                </span>
                            </div>
                        ...
                        <div class="mobile-order-date">
                            ...
                            {{ \Carbon\Carbon::parse($order->checked_out_at ?? $order->created_at)->format('F j, Y') }} &bull; {{ \Carbon\Carbon::parse($order->checked_out_at ?? $order->created_at)->format('g:i A') }}
                        </div>
                        ...
                        <div class="mobile-order-bottom">
                            <a href="{{ route('admin.orders.show', $order->id) }}" class="mobile-view-details-link">
                                {{ ($tab ?? 'new') === 'approved' ? 'View Invoice' : 'View details' }} <i class="bi bi-chevron-right"></i>
                            </a>

                            @if (($tab ?? 'new') !== 'approved' && $order->status === 'pending')
                                <div class="mobile-order-actions">
                                    <button type="button"
                                            class="mobile-reject-btn"
                                            data-bs-toggle="modal"
                                            data-bs-target="#cancelModal{{ $order->id }}">
                                        <i class="bi bi-x-circle"></i> Reject
                                    </button>

                                    <form action="{{ route('admin.orders.confirm', $order->id) }}" method="POST" class="mobile-approve-form js-approve-order-form">
                                        @csrf
    ...
    {{-- Reject modals live here (not inside .table-card, which is display:none
         on phone) so the same modal works whichever view — desktop table or
         mobile cards — the Reject button was clicked from. --}}
    @if($orders->count() && ($tab ?? 'new') !== 'approved')
        @foreach($orders as $order)
            @if($order->status === 'pending')
                <div class="modal fade remark-modal" id="cancelModal{{ $order->id }}" tabindex="-1" aria-hidden="true">
    ...
    {{-- One shared confirm modal for every Approve button (desktop table +
         mobile cards) — the JS tracks which row's form to submit. --}}
    @if($orders->count() && ($tab ?? 'new') !== 'approved')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (selectAllOrders) {
            selectAllOrders.addEventListener('click', function (e) {
                e.stopPropagation();
                document.querySelectorAll('.order-row-checkbox').forEach(cb => cb.checked = selectAllOrders.checked);
            });
        }
        ...
        // Opens the native date picker directly — no separate date-input
        // row to reveal first. #mobileDatePanel stays off-screen (clipped,
        // not display:none) so showPicker() can still find it rendered.
