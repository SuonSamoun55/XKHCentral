...
@section('content')
    @php
        ...
        $scheduledCount = $discountItems
            ->filter(fn($i) => strtolower($i->discount_status ?? '') === 'scheduled')
            ->count();
        $expiredCount = $discountItems->filter(fn($i) => strtolower($i->discount_status ?? '') === 'expired')->count();
        $activeCount = $discountItems
            ->filter(fn($i) => in_array(strtolower($i->discount_status ?? ''), ['active', 'forever']))
            ->count();

        $preparedItems = $discountItems
            ->map(function ($item) {
                ...
                return [
                    ...
                    'discount_start_date' => $item->discount_start_date
                        ? \Carbon\Carbon::parse($item->discount_start_date)->format('d M Y')
                        : '-',
    ...
    <main class="main-wrap">
            ...
            <div class="toolbar-row">
                <div class="toolbar-left">
                    <div class="search-filter-row">
                        ...
                        <button type="button" class="mobile-date-toggle-btn" id="mobileDateToggleBtn"
                    ...
                    <div class="filter-controls-row" id="filterControlsRow">
                        ...
                        <label class="date-filter"
                            onclick="document.getElementById('dateFilter').showPicker && document.getElementById('dateFilter').showPicker()">
        ...
        <div class="status-tab-row" id="statusTabRow">
            <button type="button" class="status-tab-btn active" data-status="">All
                <span>({{ $discountItems->count() }})</span></button>
            <button type="button" class="status-tab-btn" data-status="__active__">Active
                <span>({{ $activeCount }})</span></button>
            <button type="button" class="status-tab-btn" data-status="expired">Expired
                <span>({{ $expiredCount }})</span></button>
            <button type="button" class="status-tab-btn" data-status="scheduled">Scheduled
                <span>({{ $scheduledCount }})</span></button>
        </div>
        ...
        <div class="bulk-row">
            <div class="bulk-row-left">
                ...
                <button type="button" id="mobileDeleteSelectedBtn" class="js-delete-selected-btn mobile-delete-icon-btn"
            ...
            <div class="bulk-row-right">
                ...
                <div class="bulk-menu-wrap">
                    <button type="button" id="bulkMenuBtn" class="bulk-menu-btn"><i
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function showAlert(message, type = 'success') {
                ...
                el.innerHTML =
                    `<i class="bi bi-${type === 'success' ? 'check-circle-fill' : 'exclamation-triangle-fill'}"></i><span>${escapeHtml(message)}</span>`;
            ...
            function filterItems() {
                ...
                filteredItems = allItems.filter(item => {
                    const searchText = `${item.display_name} ${item.number} ${item.item_category_code}`
                        .toLowerCase();
            ...
            function bindRowEvents() {
                ...
                document.querySelectorAll('.ajax-delete-btn').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        showConfirmModal(
                            'This action is permanent and cannot be undone. This discount will be deleted.',
                            async function() {
                                ...
                                try {
                                    ...
                                    const itemIndex = allItems.findIndex(item => item.id ===
                                        id);
                                    if (itemIndex !== -1) {
                ...
                document.querySelectorAll('.row-menu-btn').forEach(button => {
                    button.addEventListener('click', function(e) {
                        ...
                        document.querySelectorAll('.row-menu-dropdown.show').forEach(d => d
            ...
            document.addEventListener('click', function() {
                document.querySelectorAll('.row-menu-dropdown.show').forEach(d => d.classList.remove(
                    'show'));
            });
            ...
            function updateBulkDeleteButtonState() {
                ...
                deleteSelectedBtns.forEach(btn => {
            ...
            nextPageBtn.addEventListener('click', function() {
                const perPage = getPerPage();
                const totalPages = perPage === 'all' ? 1 : Math.max(1, Math.ceil(filteredItems.length /
                    perPage));
            ...
            statusTabButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    statusFilter.value = ['', '__active__', 'scheduled', 'expired'].includes(
            ...
            deleteSelectedBtns.forEach(btn => btn.addEventListener('click', function() {
                ...
                showConfirmModal(
                    `This action is permanent and cannot be undone. ${ids.length} selected discount${plural} will be deleted.`,
                    async function() {
                        deleteSelectedBtns.forEach(b => {
                            b.disabled = true;
                        });
                        ...
                        results.forEach((result) => {
                            if (result.status === 'fulfilled') {
                                ...
                                const itemIndex = allItems.findIndex(item => Number(item
                                    .id) === id);
                                if (itemIndex !== -1) {
                                    allItems.splice(itemIndex, 1);
                                }
                        ...
                        if (failCount === 0) {
                            ...
                            showAlert(
                                `Deleted ${successCount} discount(s). ${failCount} failed.`,
                                'danger');
                        }
                    });
            }));
