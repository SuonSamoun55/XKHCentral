    ...
    <div class="page-wrap">
        <div class="notification-wrapper">
            ...
            <form method="GET" action="{{ route('admin.notifications.index') }}" class="filter-form">
                ...
                <div class="tab-row">
                    <div class="tabs">
                        <a href="{{ route('admin.notifications.index', array_merge(request()->except('page', 'tab'), ['tab' => 'all'])) }}"
                           class="tab-link {{ ($tab ?? 'all') === 'all' ? 'active' : '' }}">
                            All
                            <span class="tab-badge">{{ $allCount ?? 0 }}</span>
                        </a>

                        <a href="{{ route('admin.notifications.index', array_merge(request()->except('page', 'tab'), ['tab' => 'new'])) }}"
                           class="tab-link {{ ($tab ?? '') === 'new' ? 'active' : '' }}">
                            New
                            <span class="tab-badge">{{ $newCount ?? 0 }}</span>
                        </a>
                                    ...
                                    <div class="notification-content">
                                        <div class="notification-name-row">
                                            <div class="notification-name">
                                                {{ optional($notification->user)->name ?? 'Unknown User' }}
                                            </div>
                                        </div>
...
<script>
    ...
    async function checkLatestNotifications() {
        try {
            const url = `${latestNotificationUrl}?last_id=${latestNotificationId}`;
            const response = await fetch(url, {
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'application/json'
