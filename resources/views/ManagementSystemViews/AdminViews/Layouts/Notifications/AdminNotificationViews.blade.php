            ...
            <div class="notification-wrapper">
                ...
                <form method="GET" action="{{ route('admin.notifications.index') }}" class="filter-form"
                    ...
                    <div class="top-filter-row">
                        <div class="search-and-filter-row">
                            ...
                            <button type="button" class="mobile-date-toggle-btn" id="mobileDateToggleNotif"
                                aria-label="Filter by date">
                                <img src="{{ asset('images/AdminPOS/calendar (3).png') }}" alt=""
                                    class="mobile-toggle-icon">
                            </button>

                            <button type="button" class="mobile-tabs-toggle-btn" id="mobileTabsToggleNotif"
                                aria-label="Show/hide category tabs">
                                <img src="{{ asset('images/AdminPOS/filter (1).png') }}" alt=""
                                    class="mobile-toggle-icon">
                            </button>
                        </div>
                        @php
                            $authUserForChat = auth()->user();
                            $canAccessChat =
                                $authUserForChat &&
                                (strtolower((string) $authUserForChat->role) === 'admin' ||
                                    $authUserForChat->hasPermission('admin_chat'));
                        @endphp
                        <div class="top-right-tools">
                            ...
                            @endif
                            <button type="button" class="btn-send-message btn-send" data-bs-toggle="modal"
                                data-bs-target="#sendModal">
                                <img src="{{ asset('images/AdminPOS/send message.png') }}" alt=""
                                    class="btn-send-message-icon">
                ...
                <div class="utility-bar">
                    ...
                    <div class="utility-actions">
                        <button type="submit" form="markAllReadForm" class="utility-btn utility-btn-toggle">Mark all
                            read</button>
                        <button type="submit" form="deleteForm"
                            class="utility-btn delete-btn utility-btn-toggle">Delete</button>
                    </div>
                </div>
        ...
        : route('admin.notifications.show', $notification->id);
                                ...
                                <div class="notification-item {{ !$notification->is_read ? 'selected-row' : '' }}">
                                    <div class="notification-main-left">
                                        <input type="checkbox" class="notification-checkbox" name="notification_ids[]"
                                            value="{{ $notification->id }}" onclick="event.stopPropagation();">
                                        @if ($isUserContact)
                                            <button type="button" class="star-btn" title="Star"
                                        ...
                                        <div class="notification-content">
                                            <div class="notification-name-row">
                                                ...
                                                @if ($isUserContact && optional($contactUser)->id && $canAccessChat)
                                    ...
                                    <div class="notification-right">
                                        ...
                                        <div class="notification-time">
                                            {{ optional($notification->updated_at)->format('H:i') }}</div>
                ...
                <div class="pagination-wrap">
                    ...
                    <div class="pagination-controls">
                        ...
                        @endif
                        <div class="pager-current">Page {{ $notifications->currentPage() }} of
                            {{ $notifications->lastPage() }}</div>
                        @if ($nextPageUrl)
                ...
                <form action="{{ route('admin.notifications.store') }}" method="POST" id="sendNotificationForm">
                    ...
                    <div class="send-modal-header">
                            ...
                            <div class="send-recipient-area">
                                ...
                                <div class="recipient-top">
                                    ...
                                    <div class="inline-customer-search-wrap" id="inlineSearchWrap">
                                        ...
                                        <input type="text" id="customerSearchInput" class="inline-customer-input"
                                            placeholder="Search and add customer..." autocomplete="off">
                        ...
                        <div class="send-header-actions">
                            ...
                            <button type="button" class="header-icon-btn close-btn" title="Close"
                    ...
                    <div class="send-subject-row">
                        ...
                        <input type="text" name="title" class="send-subject-input" id="subjectInput"
                            placeholder="Write title..." value="{{ old('title') }}" maxlength="200">
                    ...
                    <div class="send-modal-body">
                        <div class="send-editor-wrap">
                            ...
                            <div class="editor-toolbar">
                                ...
                                <button type="button" class="toolbar-btn" id="btnBold" title="Bold"
                                    onclick="execFmt('bold','btnBold')">
                                    <i class="bi bi-type-bold"></i>
                                </button>
                                <button type="button" class="toolbar-btn" id="btnItalic" title="Italic"
                                    onclick="execFmt('italic','btnItalic')">
                                    <i class="bi bi-type-italic"></i>
                                </button>
                                <button type="button" class="toolbar-btn" id="btnUnderline" title="Underline"
                                    onclick="execFmt('underline','btnUnderline')">
                                    <i class="bi bi-type-underline"></i>
                                </button>
                                <button type="button" class="toolbar-btn" id="btnStrike" title="Strikethrough"
                                    onclick="execFmt('strikeThrough','btnStrike')">
                            ...
                            <div class="link-insert-overlay" id="linkInsertOverlay">
                                <div class="link-insert-box">
                                    ...
                                    <div class="link-insert-actions">
                                        <button type="button" class="link-cancel-btn"
                                            onclick="closeLinkOverlay()">Cancel</button>
                                        <button type="button" class="link-confirm-btn"
                                            onclick="confirmInsertLink()">Insert</button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="send-modal-footer">
                        <div class="emoji-picker" id="sendEmojiPicker">
                            <button type="button" class="emoji-option" data-emoji="😀"
                                aria-label="Grinning face">😀</button>
                            <button type="button" class="emoji-option" data-emoji="😂"
                                aria-label="Face with tears of joy">😂</button>
                            <button type="button" class="emoji-option" data-emoji="😍"
                                aria-label="Smiling face with heart eyes">😍</button>
                            <button type="button" class="emoji-option" data-emoji="👍"
                                aria-label="Thumbs up">👍</button>
                            <button type="button" class="emoji-option" data-emoji="🙏"
                                aria-label="Folded hands">🙏</button>
                            <button type="button" class="emoji-option" data-emoji="🔥" aria-label="Fire">🔥</button>
                            <button type="button" class="emoji-option" data-emoji="🎉"
                                aria-label="Party popper">🎉</button>
                            <button type="button" class="emoji-option" data-emoji="🥺"
                                aria-label="Pleading face">🥺</button>
                            <button type="button" class="emoji-option" data-emoji="❤️"
                                aria-label="Red heart">❤️</button>
                            <button type="button" class="emoji-option" data-emoji="👏"
                                aria-label="Clapping hands">👏</button>
                            <button type="button" class="emoji-option" data-emoji="😎"
                                aria-label="Smiling face with sunglasses">😎</button>
                            <button type="button" class="emoji-option" data-emoji="🤔"
                                aria-label="Thinking face">🤔</button>
                        </div>
                        <div class="footer-left-tools">
                            <button type="button" class="footer-icon-btn danger-btn" title="Clear"
                        ...
                        <div class="footer-right-tools">
                            <button type="button" class="footer-icon-btn" title="Write message"
                                onclick="focusMessageEditor()">
                            </button>
                            <button type="button" class="footer-icon-btn" id="emojiToggleBtn" title="Emoji"
                                onclick="toggleEmojiPicker(event)">
                                <i class="bi bi-emoji-smile"></i>
                            </button>
                            <button type="button" class="footer-icon-btn" title="Insert link"
                                onclick="openLinkOverlay()">
                                <i class="bi bi-link-45deg"></i>
                            </button>
...
@push('scripts')
    ...
    <script>
        ...
        function getSelectedCountEl() {
            return document.getElementById('selectedCount');
        }

        function getSelectAllEl() {
            return document.getElementById('selectAllNotifications');
        }

        function getNotificationWrapper() {
            return document.querySelector('.notification-wrapper');
        }
        ...
        function setLatestNotificationCursor() {
            latestNotificationId = Math.max(
                0,
                ...Array.from(document.querySelectorAll('.notification-item-link[data-id]')).map(el => Number(el.dataset
                    .id || 0))
            );
        ...
        document.addEventListener('submit', function(e) {
            ...
            if (form.id === 'deleteForm') {
                ...
                showConfirmModal(
                    `This action is permanent and cannot be undone. ${checkedCount} selected notification${plural} will be deleted.`,
                    function() {
                        postNotificationAction(form.action, {
                                method: 'POST',
                                body: new FormData(form),
