...
@section('content')
    ...
    <div class="chat-page {{ $activeContactId ? 'has-active-chat' : 'no-active-chat' }} {{ $activeContactId && !($hasExplicitContact ?? false) ? 'auto-selected' : '' }}"
        id="chatPage">
        @php
            $activeContactIsOnline = $activeContact ? (bool) ($activeContact->is_online ?? false) : false;
            $activeContactStatusText = $activeContact
                ? ($activeContactIsOnline
                    ? 'Online'
                    : ((string) ($activeContact->offline_duration ?? 'Offline')))
                : '';
        @endphp
        <aside class="conversation-pane">
            ...
            <div class="pane-header">
                <div class="pane-header-top">
                    <a href="{{ route('admin.notifications.index') }}" class="pane-back-btn" id="chatBackBtn"
                ...
                <div class="search-wrap">
                    ...
                    <input id="contactSearch" type="text" class="search-input" placeholder="Search conversations...">
                </div>
            </div>
                    ...
                    @forelse($contacts as $contact)
                        ...
                        <a class="contact-item {{ (int) $activeContactId === (int) $contact->id ? 'active' : '' }}"
                            ...
                            <div class="contact-avatar-wrap">
                                ...
                                <span
                                    class="contact-presence-dot {{ $contactIsOnline ? 'is-online' : 'is-offline' }}"></span>
                            ...
                            <div class="contact-text">
                                <div class="contact-name-row">
                                    ...
                                    <div class="contact-time">
                                        {{ $contact->last_message_at ? \Carbon\Carbon::parse($contact->last_message_at)->format('g:i A') : '' }}
                                    </div>
                                </div>
            ...
            <header class="mobile-chat-header">
                <div class="header-left">
                    ...
                    <div class="header-meta">
                        ...
                        <div class="header-status {{ $activeContactIsOnline ? 'is-online' : 'is-offline' }}"
                            id="mobileHeaderStatus" @if (!$activeContact) hidden @endif>
                            {{ $activeContactStatusText }}</div>
                    </div>
                </div>
                <div class="header-right">
                    <button type="button" class="header-icon" id="toggleInfoPaneMobile" title="View contact info"
                ...
                <div class="message-header-main" id="messageHeaderMain">
                    @if ($activeContact)
                        ...
                        <div class="header-info">
                            ...
                            <div class="peer-status {{ $activeContactIsOnline ? 'is-online' : 'is-offline' }}"
                                id="peerStatus">
            ...
            <div class="message-stream" id="chatBody">
                ...
                @forelse($messages as $msg)
                    @php
                        ...
                        $attachUrl = $msg->attachment_path ? '/storage/' . ltrim($msg->attachment_path, '/') : null;
                    ...
                    <div class="msg-row {{ $isMine ? 'mine' : 'other' }}">
                        ...
                        <img src="{{ $isMine ? $myAvatar : $peerAvatar }}" class="msg-avatar" alt="Avatar">

                        <div class="msg-bubble {{ $type === 'voice' && $attachUrl ? 'msg-voice' : '' }}">

                            @if ($type === 'image' && $attachUrl)
                                <img src="{{ $attachUrl }}" alt="Shared image" class="msg-image js-lightbox-trigger">
                                @if ($text !== '' && $text !== '[Image]')
                                    <div>{{ $text }}</div>
                                @endif
                        ...
                        <div class="emoji-panel" id="emojiPanel">
                            @foreach (['😀', '😂', '😍', '👍', '🙏', '🔥', '🎉', '😢', '❤️', '👏', '😎', '🤔', '😁'] as $em)
                                <button type="button" class="emoji-item"
                                    data-icon="{{ $em }}">{{ $em }}</button>
                            @endforeach
                        </div>

                        <form class="composer" id="adminChatForm" action="{{ route('admin.chat.send') }}"
                            method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="receiver_id" value="{{ $activeContactId }}">
                            <input type="file" id="imageInput" accept="image/*" style="display:none">

                            <textarea id="chatMessageInput" name="message" class="composer-input" placeholder="Type a message..."
                                rows="1"></textarea>
                            ...
                            <button type="button" class="voice-cancel-btn" id="voiceCancelButton"
                                title="Cancel recording">
                                Cancel
                            </button>

                            {{-- Send --}}
                            <button type="submit" class="send-btn"><span>Send</span><i
                            ...
                            <div class="media-grid-scroll">
                                @foreach ($sentImages as $imgMsg)
                                    @php
                                        ...
                                        $imgLink = route('admin.chat.index', [
                                            'user_id' => $activeContact->id,
                                            'date' => $imgDate,
                                        ]);
                                    @endphp
                                    <button type="button" class="media-thumb-link js-lightbox-trigger"
                            ...
                            <div class="media-grid-scroll">
                                @foreach ($sharedMedia as $media)
                                    ...
                                    <button type="button" class="media-thumb-link js-lightbox-trigger"
                                        ...
                                        <img class="media-thumb" src="{{ $imgUrl }}" alt="Media">
                                    </button>
                                @endforeach
                            </div>
                        </div>
...
@push('scripts')
    <script>
        (function() {
            'use strict';
            let form, input, attachButton, imageInput, emojiButton, emojiPanel, voiceButton, voiceCancelButton,
                composerHint, removeImgBtn;
            ...
            function attachLightbox(scope) {
                ...
                const imageSources = triggers.map(function(trigger) {
                    return trigger.dataset.fullSrc || trigger.getAttribute('src') || trigger.querySelector(
                        'img')?.getAttribute('src') || '';
                }).filter(Boolean);
                triggers.forEach(function(trigger, index) {
                    trigger.addEventListener('click', function(e) {
                        e.preventDefault();
                        const src = trigger.dataset.fullSrc || trigger.getAttribute('src') || trigger
                ...
                scope.querySelectorAll('.voice-player-row').forEach(function(row) {
                    ...
                    btn.addEventListener('click', function() {
                        document.querySelectorAll('.voice-audio-src').forEach(function(a) {
                            if (a !== audio && !a.paused) {
                                a.pause();
                                const otherIcon = a.closest('.voice-player-row')?.querySelector(
                                    '.voice-toggle-btn i');
            ...
            function resetMessageState() {
                ...
                lastRenderedDate = '';
                lastId = 0;
                chatBody?.querySelectorAll('.voice-audio-src').forEach(function(a) {
                    a.pause();
                });
            ...
            function renderMessages(messages) {
                ...
                if (!Array.isArray(messages) || !messages.length) {
                    chatBody.innerHTML =
                        '<div class="text-muted" id="emptyChatText" style="font-size:13px;color:#9ca3af;align-self:center;margin-top:40px">Select a conversation to start chatting.</div>';
                    return;
                }
            ...
            function renderHeader(contact) {
                ...
                if (mobileHeaderStatus) {
                    if (contact) {
                        ...
                        mobileHeaderStatus.textContent = contact.status_text || (contact.is_online ? 'Online' :
                            'Offline');
                ...
                <div class="header-info">
                    ...
                    messageHeaderMain.innerHTML =
                        `<div class="header-info"><div class="peer-name">Admin Chat</div></div>`;
            ...
            function renderInfoPane(contact, sharedMedia) {
                ...
                const infoRows = [
                    contact.phone ?
                    `<div class="rp-info-row"><i class="bi bi-telephone"></i><span>${escapeHtml(contact.phone)}</span></div>` :
                    '',
                    contact.email ?
                    `<div class="rp-info-row"><i class="bi bi-envelope"></i><span>${escapeHtml(contact.email)}</span></div>` :
                    '',
                    contact.location ?
                    `<div class="rp-info-row"><i class="bi bi-geo-alt"></i><span>${escapeHtml(contact.location)}</span></div>` :
                    '',
                ].join('');
            ...
            async function startVoiceRecording() {
                ...
                try {
                    ...
                    mediaRecorder = recOpts ? new MediaRecorder(mediaStream, recOpts) : new MediaRecorder(
                        mediaStream);
                    recordedChunks = [];
                    shouldSend = true;
                    ...
                    mediaRecorder.onstop = async function() {
                        ...
                        if (mediaStream) mediaStream.getTracks().forEach(function(t) {
                            t.stop();
                        });
                        ...
                        selectedVoiceFile = new File([blob], `voice-${Date.now()}.${ext}`, {
                            type: mime
                        });
                        selectedImageFile = null;
            ...
            function applyContactSearchFilter() {
                ...
                contactList?.querySelectorAll('.contact-item').forEach(function(item) {
                    const match = (item.dataset.name || '').includes(kw) || (item.dataset.last || '').includes(
                        kw);
            ...
            toggleInfoPane?.addEventListener('click', function() {
                ...
                toggleInfoPane.querySelector('i').className = hidden ? 'bi bi-layout-sidebar-inset-reverse' :
