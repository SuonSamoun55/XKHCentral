...
@push('styles')
<link rel="stylesheet" href="{{ asset('/css/views/POSViews/POSAdminViews/AdminNotification/NotificationsViews.css') }}?v={{ filemtime(public_path('css/views/POSViews/POSAdminViews/AdminNotification/NotificationsViews.css')) }}">
@endpush
@section('content')
@php
    ...
    $hasRealAvatar = ($contactUser && !empty($contactUser->profile_image_display))
    ...
    if ($isOrderNotification) {
        ...
        $orderStatusMap = [
            'cancelled' => ['class' => 'cancelled', 'pillIcon' => 'bi-x-circle-fill', 'alertIcon' => 'bi-cart-x-fill', 'label' => 'Cancelled', 'note' => 'Order cancelled'],
            'pending' => ['class' => 'pending', 'pillIcon' => 'bi-hourglass-split', 'alertIcon' => 'bi-cart-x-fill', 'label' => 'Pending', 'note' => 'Order pending'],
        ];
        ...
        <div class="detail-wrapper">
            <div class="detail-header">
                <div class="header-title-group">
                    <a href="{{ route('admin.notifications.index') }}" class="header-back-arrow" aria-label="Back" id="detailBackArrow">
                    ...
                    <div class="order-items-card">
                            ...
                            <table class="order-items-table">
                                ...
                                <tbody>
                                    @foreach ($orderItems as $line)
                                        @php
                                            $lineImage = optional($line->itemVariant)->image_url
                                                ?? optional($line->item)->custom_image_url
                                                ?? optional($line->item)->image_url;
                                            $lineImage = $lineImage
                                                ? (str_starts_with($lineImage, 'http') ? $lineImage : asset($lineImage))
                                                : asset('images/no-image.png');
                                            $lineVatPercent = (!empty(optional($line->item)->price_includes_tax))
                                                ? 0
                                                : max(0, (float) (optional($line->item)->vat_percent ?? 0));
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $lineDiscountPercent > 0 ? rtrim(rtrim(number_format($lineDiscountPercent, 2), '0'), '.') . '%' : '—' }}</td>
                        ...
                        @if ($orderItems->hasPages())
                            <div class="order-items-pagination">
                                <a
                                    class="page-arrow {{ $orderItems->onFirstPage() ? 'disabled' : '' }}"
                                    href="{{ $orderItems->onFirstPage() ? '#' : $orderItems->previousPageUrl() }}"
                                    aria-label="Previous page"
                                ><i class="bi bi-chevron-left"></i></a>

                                @for ($p = 1; $p <= $orderItems->lastPage(); $p++)
                                    <a class="page-num {{ $p === $orderItems->currentPage() ? 'active' : '' }}" href="{{ $orderItems->url($p) }}">{{ $p }}</a>
                                @endfor

                                <a
                                    class="page-arrow {{ $orderItems->hasMorePages() ? '' : 'disabled' }}"
                ...
                <div class="order-detail-grid stock-detail-grid">
                    <div class="stock-left-col">
                        ...
                        <div class="stock-alert-card">
                            ...
                            <p class="stock-alert-message">{{ trim($cleanMessage) !== '' ? trim($cleanMessage) : 'No further details available.' }}</p>
                        ...
                        <div class="affected-product-card">
                            ...
                            @php
                                $stockItemImage = $stockItem->image_url
                                    ? (str_starts_with($stockItem->image_url, 'http') ? $stockItem->image_url : asset($stockItem->image_url))
                                    : asset('images/no-image.png');
                            @endphp
                            <a href="{{ route('store.management.products.detail', $stockItem->id) }}" class="affected-product-row">
                                ...
                                <div class="affected-product-info">
                                    ...
                                    <div class="affected-product-meta">SKU: <strong>{{ $stockItem->number ?: 'N/A' }}</strong></div>
                                    @if ($stockItem->item_category_code)
                                        <div class="affected-product-meta">Category: <strong>{{ $stockItem->item_category_code }}</strong></div>
                                    @endif
                                </div>
                            </a>
                        </div>
                                ...
                                <div class="receipt-item-info">
                                    <div class="receipt-item-name">
                                        ...
                                        @if ($line->variant_description)
                                            <span class="receipt-item-size">&middot; {{ $line->variant_description }}</span>
                                        @endif
                                    </div>
                                    <div class="receipt-item-meta-row">
                                        <span>VAT: {{ $lineVatPercent }}%</span>
                                        <span>Discount: {{ $lineDiscountPercent > 0 ? rtrim(rtrim(number_format($lineDiscountPercent, 2), '0'), '.') . '%' : '0' }}</span>
            ...
            <div class="bottom-actions">
                <a href="{{ route('admin.notifications.index') }}" class="back-btn secondary" id="detailBackToList">Back to List</a>
...
(function () {
    function showAlert(message, type) {
        ...
        el.innerHTML = '<i class="bi bi-' + (type === 'success' ? 'check-circle-fill' : 'exclamation-triangle-fill') + '"></i><span></span>';
    ...
    function triggerDelete() {
        showConfirmModal('This action is permanent and cannot be undone. This notification will be deleted.', function () {
            deleteBtns.forEach(function (btn) { btn.disabled = true; });
            ...
            fetch(deleteForm.action, {
                ...
                .catch(function (err) {
                    deleteBtns.forEach(function (btn) { btn.disabled = false; });
