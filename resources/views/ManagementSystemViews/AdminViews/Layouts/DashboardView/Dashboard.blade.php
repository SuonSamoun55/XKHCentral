    ...
    <section class="hero-row">
        <div class="hero-card">
            ...
            <span class="hero-label">
                <span class="hero-card-title">Products</span><span class="hero-colon">: </span><span class="hero-card-value">{{ number_format($totalProducts) }}</span>
            </span>
        </div>

        <div class="hero-card soft-card" data-href="{{ route('users.index', ['status' => 'connected']) }}">
            <div class="hero-icon-box">
                <img src="{{ asset('images/management/Total Customers.png') }}" alt="Total Customers" onerror="this.style.display='none'">
            </div>
            <span class="hero-label">
                <span class="hero-card-title">Total Customers</span><span class="hero-colon">: </span><span class="hero-card-value">{{ number_format($totalCustomers) }}</span>
            </span>
        </div>
        <div class="hero-card soft-card" data-href="{{ route('users.index', ['active' => 'online']) }}">
            <div class="hero-icon-box">
                <img src="{{ asset('images/management/Online Customers.png') }}" alt="Online Customers" onerror="this.style.display='none'">
            </div>
            <span class="hero-label">
                <span class="hero-card-title">Online Customers</span><span class="hero-colon">: </span><span class="hero-card-value">{{ number_format($onlineCustomers) }}</span>
            </span>
        </div>
    </section>
    ...
    <div class="dashboard-grid">
        ...
        <section class="panel report-panel">
            <div class="panel-head">
                ...
                <div class="panel-controls">
                    ...
                    <select class="filter-select" id="reportYearSelect" style="{{ $reportPeriod === 'year' ? '' : 'display:none;' }}">
            ...
            <div class="chart">
                ...
                <div class="chart-bars {{ count($chartData) > 14 ? 'dense' : '' }}" id="chartBars">
                    @foreach ($chartData as $bar)
                        <div class="chart-col">
                            <div class="chart-bar {{ $bar['muted'] ? 'muted' : '' }}" style="height:{{ $yAxisMax > 0 ? max(4, ($bar['value'] / $yAxisMax) * 100) : 4 }}%" data-value="{{ $bar['value'] }}" data-label="{{ $bar['label'] }}" tabindex="0"></div>
            ...
            <div class="panel-head">
                <div class="panel-title-icon">
                    <span class="icon-badge">
                        ...
                        <img src="{{ asset('images/management/Top Sale.png') }}" alt="" onerror="this.style.display='none'">
                    </span>
                    <h2>Top Products</h2>
                </div>
                <div class="panel-controls">
                    <select class="filter-select" id="productsPeriodSelect">
                        ...
                        <option value="month" {{ $topProductsPeriod === 'month' ? 'selected' : '' }}>This Month</option>
                ...
                @forelse ($topProducts as $p)
                    <li class="product-item">
                        <span class="product-thumb-wrap">
                            <span class="product-thumb-fallback">{{ strtoupper(mb_substr(trim($p['name'] ?? ''), 0, 1)) ?: '?' }}</span>
                            @if (!empty($p['thumb']))
                                <img class="product-thumb" src="{{ $p['thumb'] }}" alt="{{ $p['name'] }}" onerror="this.style.display='none';">
                            @endif
                        </span>
        ...
        <section class="panel notification-panel soft-card" data-href="{{ route('admin.notifications.index') }}">
            <div class="panel-head">
                <div>
                    <h2>Notification</h2>
                    <p class="panel-sub">{{ number_format($unreadNotificationCount) }} unread {{ Str::plural('message', $unreadNotificationCount) }}</p>
                </div>
            </div>
            ...
            <div class="stat-cards">
                <div class="stat-card">
                    <span class="stat-icon">
                        {{-- Swap this src for your own icon --}}
                        <img src="{{ asset('/images/management/Total Income.png') }}" alt="" onerror="this.style.display='none'">
                    </span>
                    <div class="stat-info">
                ...
                <div class="stat-card">
                    <span class="stat-icon">
                        {{-- Swap this src for your own icon --}}
                        <img src="{{ asset('images/management/Total Confirm.png') }}" alt="" onerror="this.style.display='none'">
                    </span>
                    <div class="stat-info">
                ...
                <div class="stat-card">
                    <span class="stat-icon">
                        <img src="{{ asset('images/management/Pending Products.png') }}" alt="" onerror="this.style.display='none'">
                    </span>
                    <div class="stat-info">
...
document.getElementById('statsPeriodSelect')?.addEventListener('change', function () {
    ...
    fetch(`{{ route('admin.dashboard.overview-stats') }}?period=${encodeURIComponent(period)}`, {
        ...
        .then(res => res.json())
        .then((stats) => {
            document.getElementById('statIncomeValue').textContent = '$' + Math.round(stats.totalIncome).toLocaleString();
            document.getElementById('statConfirmedValue').textContent = Number(stats.totalConfirmedOrders).toLocaleString();
            document.getElementById('statPendingValue').textContent = Number(stats.pendingProductCount).toLocaleString();
        })
...
function loadReportChart() {
    ...
    fetch(url, {
        ...
        .then(res => res.json())
        .then(({ chartData, yAxisSteps, yAxisMax }) => {
