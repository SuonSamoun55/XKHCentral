            ...
            <div class="top-bar">
                <div class="left-tools">
                    <div class="user-search-box">
                        ...
                        <input type="text" id="staffSearch" class="user-search-input" placeholder="Search by name or email">
                    </div>
                </div>
                ...
                <div class="table-scroll">
                    <table class="table align-middle">
                        ...
                        <tbody id="staffTableBody">
                            @forelse($staff as $member)
                                @php
                                    ...
                                    $lastSeenText = $member->last_seen_at
                                        ...
                                        : '-';
                                @endphp
                                <tr class="staff-row" data-name="{{ strtolower($member->name) }}" data-email="{{ strtolower($member->email ?? '') }}">
                                    <td>
                                        <div class="avatar-cell">
                                            <div class="avatar-wrap">
                                                @if($member->profile_image_display)
                                                    <img src="{{ $member->profile_image_display }}" alt="" class="avatar-image"
                                    ...
                                    <td>
                                        <div class="action-icons">
                                            ...
                                            <button type="button"
                                                ...
                                                data-bs-toggle="modal"
                                                data-bs-target="#staffPasswordModal"
                                                data-id="{{ $member->id }}"
                                                data-name="{{ $member->name }}"
                                                class="open-staff-password">
                            ...
                            @empty
                                <tr id="noStaffRow">
                                    <td colspan="7" class="empty-text">No staff accounts yet. Create one to get started.</td>
                                </tr>
                            @endforelse
                        ...
                        <div class="mb-3">
                            ...
                            <select name="company_id" class="form-select custom-input">
                                ...
                                @foreach($companies as $companyOption)
                                    <option value="{{ $companyOption->id }}">{{ $companyOption->display_name ?? $companyOption->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @else
        ...
        <div class="modal-content custom-user-modal">
            <form method="POST" id="staffEditForm">
                ...
                <div class="modal-body custom-modal-body">
                    <div class="mb-3">
                        <label class="form-label custom-label">Name:</label>
                        <input type="text" name="name" id="staffEditName" class="form-control custom-input" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label custom-label">Email:</label>
                        <input type="email" name="email" id="staffEditEmail" class="form-control custom-input" required>
                    </div>
            ...
            <form method="POST" id="staffPasswordForm">
                ...
                <div class="modal-body custom-modal-body">
                    @error('current_password')
                        <div class="pw-error mb-2">{{ $message }}</div>
                    @enderror

                    <div class="mb-3">
                        <label class="form-label custom-label" for="staffCurrentPassword">Your Current Password:</label>
                        <input type="password" name="current_password" id="staffCurrentPassword" class="form-control custom-input" placeholder="Confirms it's really you making this change" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label custom-label" for="staffNewPassword">New Password:</label>
                        <input type="password" name="password" id="staffNewPassword" class="form-control custom-input" required>
                        <div id="staff-password-strength" class="password-strength"></div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label custom-label" for="staffNewPasswordConfirm">Confirm New Password:</label>
                        <input type="password" name="password_confirmation" id="staffNewPasswordConfirm" class="form-control custom-input" required>
                    </div>
                </div>

                <div class="modal-footer custom-modal-footer">
                    <button type="button" class="btn modal-cancel-btn" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn modal-save-btn" id="staffPasswordSubmitBtn">Update Password</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="pw-confirm-overlay" id="pwConfirmOverlay">
    <div class="pw-confirm-box">
        ...
        <p class="pw-confirm-text" id="pwConfirmText">Are you sure you want to change this staff member's password?</p>
...
@if(session('success') && session('new_password'))
    <div class="pw-success-overlay show" id="pwSuccessOverlay">
        <div class="pw-success-box">
            ...
            <p class="pw-success-text">Share this with {{ session('password_updated_for', 'the staff member') }} — it won't be shown again.</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.open-staff-password').forEach(function (btn) {
        btn.addEventListener('click', function () {
            const id = btn.dataset.id;
            document.getElementById('staffPasswordForm').action = '/staff/' + id + '/password';
            document.getElementById('staffPasswordTargetName').textContent = btn.dataset.name || '';
            document.getElementById('pwConfirmText').textContent =
                'Are you sure you want to change the password for ' + (btn.dataset.name || 'this staff member') + '?';
        });
    });
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            rows.forEach(function (row) {
                const matches = !term || row.dataset.name.includes(term) || row.dataset.email.includes(term);
